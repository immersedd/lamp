...
class TCNNDS(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        if self.aggregator in ['mean', 'max']:
            combined_emb = self.fusion(stacked_emb)
        elif self.aggregator == 'deepsets':
            combined_emb = self.deep_set_aggregator(stacked_emb)
        else:
            combined_emb = torch.mean(stacked_emb, dim=1)
...
class DeepSetAggregator(nn.Module):
    ...
    def __init__(self, input_dim, phi_hidden_dims, phi_output_dim, rho_hidden_dims, output_dim, aggr='mean'):
